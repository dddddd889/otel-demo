...
异步消息使用另一条递进学习线，见 [异步消息传播学习路线](../async-messaging/README.md)。其中 RabbitMQ 基础消息流已在工作区实现，但尚未形成 Git commit。
...
当前仅有独立 RabbitMQ Producer/Consumer 基础流。`payment-service`、checkout 发布订单、异步 Trace Context、Producer/Consumer Span、Span Link、重试和死信队列仍未实现，不能写成已完成能力。
